...
export function useSchemaSearchTab6Utils() {
  // 그리드 정보 조회
  const getGridInfo = (tab6GridId) => async () => {
    try {
      const userGridData = await getUserGridSetting(tab6GridId.value);
  ...
  const onSearchEnter = (params) => async (value) => {
    ...
    if (searchType.value === 'natural-query' && value !== '') {
      ...
      rowData.value = [];
      const triggerResearchResult = await getTriggerList(researchQueryData);
      console.log('triggerResearchResult : ', triggerResearchResult);
      searchDataBinding(triggerResearchResult);
    } else {
      const researchQueryData = {
        instituteId: useMetaMngInstId,
        informationSystemId: useInfoSysId,
  ...
  const onFilterWindowClosed = (params) => (filterSet) => {
    ...
    console.log('gridStorage.MFGRD065 : ', gridStorage.MFGRD065);
    columnDefs.value = gridStorage.MFGRD065;

    gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD065);
  };

  // 사용자 그리드 설정 저장
  const onSetUserGridSetting = (params) => async () => {
    const { columnDefs, tab6GridId } = params;
    ...
    await setUserGridSetting(tab6GridId.value, newGridStting);
  };

  // 검색 조건 초기화
  const onSearchRemove = (params) => async () => {
    const { tab6GridId, columnDefs, gridApi, uiStore } = params;

    console.log('onSearchRemove 함수 실행 ===');

    // 그리드 기본값 호출
    const gridDefaultData = await getGridDefaultData(tab6GridId.value);
    console.log('gridDefaultData : ', gridDefaultData);

    // 기본값 설정
    await setUserGridSetting(tab6GridId.value, gridDefaultData);
    ...
    const gridStorage = JSON.parse(getGridInfoFromStorage());
    gridStorage[tab6GridId.value] = transformedData;
    saveGridInfoToStorage(gridStorage);

    uiStore.setGridColumnDefs(tab6GridId.value, columnDefs.value);
