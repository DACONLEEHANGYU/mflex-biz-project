...
export function useSchemaSearchTab6Handlers() {
  ...
  const handleColumnStateChanged = (params) => (newColumnState) => {
    ...
    // localStorage에서 gridData json 파싱, MFGRD065에 대한 값 변경
    const gridStorage = JSON.parse(getGridInfoFromStorage());
    gridStorage.MFGRD065 = newColumnFcDefs;
    // 로컬스토레지에 저장
    saveGridInfoToStorage(gridStorage);
    // 스토어에 저장
    uiStore.setGridColumnDefs('MFGRD065', newColumnFcDefs);
  };
  ...
  const handleSortChanged = (params) => async (newSortedState) => {
    ...
    if (sortState.value.length == 0) {
      ...
      const sortData = await getTriggerList(sortResearchQuery);
