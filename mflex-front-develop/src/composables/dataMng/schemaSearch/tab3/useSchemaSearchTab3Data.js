...
export function useSchemaSearchTab3Data() {
  // 기본키 데이터 조회 함수
  const fetchData = (params) => async (filterSet) => {
    ...
    const responseData = await getPrimaryKeyList(researchQuery);
    const pkList = responseData.items;

    resultCount.value.count = responseData.searchCount;
    resultCount.value.total = responseData.totalCount;

    console.log('primaryKeyList : ', responseData);

    const tempData = pkList.map((item, index) => ({
      id: index,
      no: index + 1,
      instituteId: item.instituteId,
      informationSystemId: item.informationSystemId,
      databaseId: item.databaseId,
      primaryKeyCompositionContent: item.primaryKeyCompositionContent,
    ...
    if (rowData.value.length > 0) {
      nextTick(() => {
        ...
        setPrimaryKeyData(firstRowData.data);
      });
    }
  };
  ...
  const addGridRowData = (params) => async (lastItem) => {
    ...
    try {
      ...
      const response = await getPrimaryKeyList(researchQuery);
      const pkList = response.items;

      const newGridData = pkList.map((item, index) => ({
  ...
  // 기본키 조회 (필터 리서치)
  const searchDataBinding = (params) => (pkResearchResult) => {
    const { resultCount, rowData, setPrimaryKeyData, agGrid, emit } = params;

    console.log('pkResearchResult : ', pkResearchResult);

    const pkList = pkResearchResult.items;

    if (pkList.length < 0) {
      resultCount.value.count = 0;
      resultCount.value.total = 0;
    } else {
      resultCount.value.count = pkResearchResult.searchCount;
      resultCount.value.total = pkResearchResult.totalCount;
