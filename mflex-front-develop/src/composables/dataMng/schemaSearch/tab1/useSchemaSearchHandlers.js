// composables/dataMng/schemaSearch/useSchemaSearchHandlers.js
...
export function useSchemaSearchHandlers() {
  ...
  const handleColumnStateChanged = (params) => (newColumnState) => {
    const { gridApi, columnDefs, tab1GridId } = params;

    // ES6 import로 변경된 함수 사용
  ...
  const handleSortChanged = (params) => async (newSortedState) => {
    ...
    if (sortState.value.length == 0) {
      ...
      const termSortData = await getTableList(sortResearchQuery);
