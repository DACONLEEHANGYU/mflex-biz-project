<template>
  <div class="pop-window">
    <div class="window-header">도메인명 검색</div>
      ...
      <div class="window-content pt10" style="height: 440px">
        <div class="input-top">
          <div class="title-top">
            <div class="title-s">
              도메인명 목록
              <AppTooltip :htmlContent="getTooltipById('searchDomain')">
              </AppTooltip>
            </div>
          </div>
        </div>
        <div class="search-top">
          <div class="search-title">도메인분류명 :</div>
        ...
        <div class="tab-comtent__row h-450">
          ...
          >
            <template #left>
              <div class="grid-wrap domain-class-grid" style="height: 300px">
                <div class="grid-top">
  ...
  export default {
    ...
    methods: {
      ...
      onRowClicked(value) {
        ...
        // 모든 행의 데이터를 순회하면서 gridApi를 통해 직접 업데이트
        if (this.domainNameGrid && this.domainNameGrid.gridApi) {
          // this.domainNameGrid.gridApi.forEachNode((node) => {
          //   node.setDataValue('selected', node.data.id === value.id);
          // });

          // 그리드 새로고침
          this.domainNameGrid.gridApi.refreshCells({
            columns: ['selected'],
            force: true,
          });
        }

        console.log('this.firstSelectRow : ', this.firstSelectRow);

        const selectDomain = {
          domainName: value.domainName,
          domainClassName: this.firstSelectRow.domainClassName,
        };

        this.selectedRow = selectDomain;

        const rowNode = this.domainNameGrid.gridApi.getRowNode(value.id);
      ...
      //   matchedData.englishAbbreviationName = params.selectTermAbbreviationName;
    ...
    setup(props, { emit }) {
      ...
      const fetchData = async () => {
        ...
        console.log('domainClassList : ', domainClassList);
        ...
        const domainSimpleParam = {
          ...
          domainClassName: domainClassData[0].domainClassName,
        ...
        fetchDomainName(domainSimpleList);
      };
      ...
      /* 도메인분류 그리드 선택 */
      const selectDomainClass = async (value) => {
        console.log('value : ', value);
        firstSelectRow.value = value;
        const dialogContainer = document.querySelector('.domain-class-grid');
      ...
      const firstRowSelectedEvent = () => {
        if (domainClassRowData.value.length > 0) {
          nextTick(() => {
            const dialogContainer =
              document.querySelector('.domain-class-grid');
            ...
            if (firstRowData) {
              console.log('event-firstRowData', firstRowData);
      ...
      const fetchDomainName = async (domainSimpleList) => {
        console.log('fetchDomainName domainSimpleList', domainSimpleList);

        const domains = domainSimpleList.data;
        let domainData = [];
        for (let i = 0; domains.length > i; i++) {
          domainData.push({
            id: i,
        ...
        domainRowData.value = domainData;
      };
