            ...
            <div class="tab-comtent__row h-450">
              <div class="tab-inner grid-tab-height">
                <div class="upload-container">
                    ...
                    <div class="upload-area">
                      ...
                      <div class="result-area">
                        <span class="result-text"
                          ...
                          {{ resultCount.success }}건 /<span style="color: red">
                            실패 : {{ resultCount.fail }}건
                          </span>
                        </span>
                      </div>
                      <div class="file-area">
                  ...
                  <div class="grid-wrap domain-grid-area">
...
<script>
  ...
  import { getDomainUplaodFile } from '@/utils/mflexApi/dictionaryManagementApi.js';
  import { uploadDomainFileV2 } from '@/utils/mflexApi/dictionaryMng/dictionaryV2Api.js';
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const onConfirm = async () => {
        console.log('domainFormData : ', domainFormData);
        console.log('domainFormData.value : ', domainFormData.value);

        const response = await uploadDomainFileV2(domainFormData.value);
        ...
        const file = domainFormData.value.get('file');
        ...
        reader.onload = (event) => {
          ...
          const errorResult = failExcelData.map((row, index) => {
            return {
              ...
              logicalDataTypeCode: row['논리데이터타입'],
              dataLength: row['데이터길이'],
              dataDecimalPointLength: row['데이터소수점길이'],
              explain: row['도메인설명'],
      ...
      const downloadErrorFile = () => {
        ...
        link.download = `도메인업로드_오류내역_(${sysdate}).xlsx`; // 다운로드할 파일명 설정
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
