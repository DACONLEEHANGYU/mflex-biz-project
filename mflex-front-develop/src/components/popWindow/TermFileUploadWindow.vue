<template>
  <div class="pop-window">
    <div class="window-header">용어 일괄등록</div>
                    ...
                    <div class="upload-area">
                      ...
                      <div class="result-area">
                        <span class="result-text"
                          ...
                          {{ resultCount.success }}건 /
                          <span style="color: red"
                            >실패 : {{ resultCount.fail }}건</span
                          >
                        </span>
                      </div>
                      <div class="file-area">
                        ...
                        <div class="btn-area">
                          ...
                          />
                          <a
                            :href="`${apiUrl}/api/v1/download/term-template`"
                            class="btn-s"
                            >양식내려받기</a
                          >
                          <button
...
<script>
  ...
  import { getTermUploadFile } from '@/utils/mflexApi/dictionaryManagementApi.js';
  import { uploadTermFileV2 } from '@/utils/mflexApi/dictionaryMng/dictionaryV2Api.js';
  export default {
    components: {
      AppWarningTooltip,
      StatusCellRenderer,
    ...
    setup(props, { emit }) {
      ...
      const handleFileUpload = (event) => {
        ...
        if (file) {
          ...
          formData.append('dictionaryId', useDctnryId); // 직접 form-data 필드로 추가
          formData.append('file', file);

          termFormData.value = formData;

          // 엑셀파일을 읽는 부분 (기존 코드와 동일)
      ...
      const onConfirm = async () => {
        console.log('termFormData : ', termFormData);
        console.log('termFormData.value : ', termFormData.value);

        const response = await uploadTermFileV2(termFormData.value);

        console.log('uploadResponse : ', response);

        const failRows = response.data.results.filter((item) => !item.success);

        resultCount.total = response.data.results.length;

        console.log('failRows : ', failRows);

        const file = termFormData.value.get('file');
        ...
        reader.onload = (event) => {
          ...
          const failExcelData = excelData
            .map((row, index) => {
              ...
              if (failItem) {
                ...
                return {
                  ...row,
                  오류내역: failItem.errors[0].message || '알 수 없는 오류',
                };
              }
              return null;
            })
          ...
          const errorResult = failExcelData.map((row, index) => {
            return {
              ...
              jobType: row['작업구분'],
              termName: row['용어명'],
              termEngAbbreviationName: row['용어영문약어명'],
              termEnglishName: row['용어영문명'],
              domainName: row['도메인명'],
              standardDivision: row['표준여부'],
              termExplain: row['용어설명'],
              codeType: row['코드유형'],
              individualCodeName: row['개별코드'],
              unityCodeName: row['통합코드'],
              dataUnitName: row['데이터단위'],
              dataPermissionValue: row['데이터허용값'],
              storageFormatContext: row['저장형식'],
              expressionFormatContext: row['표현방식'],
              nonStandardReasonCode: row['비표준사유코드'],
              nonStandardReasonContents: row['비표준사유기타내용'],
              manageDepartmentName: row['관리부서'],
              taskFieldName: row['업무분야'],
              revisionDate: row['제개정일자'],
      ...
      const downloadErrorFile = () => {
        ...
        link.download = `용어업로드_오류내역_(${sysdate}).xlsx`; // 다운로드할 파일명 설정
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
