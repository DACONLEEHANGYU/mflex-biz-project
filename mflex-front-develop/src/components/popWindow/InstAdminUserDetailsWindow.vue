<template>
      ...
      <div class="window-content pt15">
        <div class="tab-comtent__row">
          <div class="window-top">
            ...
            <!-- <button class="btn-s" @click="onUserApproval">승인</button> -->
            <!-- <button class="btn-s" @click="onUserLockConfirim">
              잠금/잠금해제
            </button> -->
  ...
  >
    <InstAdminEditUserDetailsWindow
...
<script setup>
  ...
  import InstAdminEditUserDetailsWindow from '@/components/popWindow/InstAdminEditUserDetailsWindow.vue';
  import {
  ...
  const UserResourceTab = defineAsyncComponent(() =>
    import('@/components/popWindow/InstAdminUserResourceTab.vue')
  );

  const currentTab = shallowRef(UserBasicInfoTab);
  const tabList = reactive(['기본정보', '리소스']);
