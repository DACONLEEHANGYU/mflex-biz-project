<template>
  <div class="pop-window">
    <div class="window-header">사전 변경</div>
    ...
    methods: {
      ...
      async onConfirm() {
        const resourceInfo = {
          ...
          oldDictionaryId: this.dictionaryId,
          newDictionaryId: this.selectedRow.dictionaryId,
        };

        const response = await updateResourceDictionary(resourceInfo);
    ...
    setup(props, { emit }) {
      ...
      const fetchData = async () => {
        ...
        for (let i = 0; i < dictionaryList.length; i++) {
          sampleData.push({
            dictionaryId: dictionaryList[i].id,
            dictionaryName: dictionaryList[i].name,
            radio: '',
          });
        }
