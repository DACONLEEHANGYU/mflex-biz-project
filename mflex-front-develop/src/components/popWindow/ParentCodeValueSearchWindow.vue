<template>
  <div class="pop-window">
    <div class="window-header">상위코드값 검색</div>
  ...
  export default {
    ...
    methods: {
      ...
      async onRowClicked(value) {
        ...
        const response = await this.getParentCodeData(parentCodeParams);

        console.log('parentCode response', response);
      ...
      onConfirm() {
        this.$emit('confirm', this.selectParentCodeValue);
        this.$emit('close');
      },
    },
    beforeMount() {
    ...
    setup(props, { emit }) {
      ...
      const selectParentCodeValue = ref(null);
      ...
      const getUnityCodeKoreanName = async () => {
        ...
        const response = await getParentCodeKoreanNameList(
        ...
        await getParentCodeData(parentCodeParams);
      };

      getUnityCodeKoreanName();

      // 상위코드 조회
      const getParentCodeData = async (data) => {
