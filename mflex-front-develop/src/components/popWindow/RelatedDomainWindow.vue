<template>
  <div class="pop-window">
    <div class="window-header">연관도메인 내역</div>
            ...
            <div class="grid-top">
              ...
              <div class="top-row" style="border-top: none">
                <div class="top-apptext">
                  ...
                  &nbsp;&nbsp;
                  <AppStateText
                    v-model="selectDomainClass.value.domainClassName"
                  />
                </div>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    setup(props, { emit }) {
      console.log('props.selectDomainClass : ', props.selectDomainClass);
      ...
      const fetchData = async () => {
        const list = props.propsRelatedDomainList;
        ...
        for (let i = 0; i < list.length; i++) {
          sampleData.push({
            ...
            domainName: [
              {
                id: 0,
                type: list[i].domain.dictionary.type.name,
                label: list[i].domain.name,
                color: list[i].domain.dictionary.type.fontColor,
                bgColor: list[i].domain.dictionary.type.backgroundColor,
                size: 60,
              },
            ],
            explain: list[i].domain.explain,
          });
        }
