  ...
  methods: {
    ...
    async onSearchEnter(value) {
      ...
      if (this.searchType === 'natural-query' && value !== '') {
        ...
        const termResearchResult = await getResaerchTerm(researchQuery);
  ...
  setup(props, { emit }) {
    ...
    onBeforeMount(async () => {
      ...
      if (!gridStorage[tab1GridId.value]) {
        ...
        try {
          ...
          // localStorage에 gridData json 파싱, MFGRD017에 대한 값 변경
    ...
    const getTermRowData = (getTermSearchResult) => {
      ...
      try {
        ...
        const terms = getTermSearchResult.data.terms;
