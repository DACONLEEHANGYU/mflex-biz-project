<template>
  ...
  <!-- 표준화 단어추출-->
  <AppWindow
    :view="extractWordWindowView"
    @close="onCloseExtractWordWindow"
    width="700px"
    height="auto"
  >
    <ConfirmWindow
      :popInfo="popInfo"
      @confirm="onExtractWord"
      @close="onCloseExtractWordWindow"
    />
  </AppWindow>
  ...
  >
    <ApprovalRequestWindow
      :type="'word-synchronize'"
      :selectedData="selectWordGridData"
...
<script>
  ...
  import { getExtractWord } from '@/utils/mflexApi/actualizing/actualizingApi';
  ...
  export default {
    ...
    methods: {
      ...
      async onRowClicked(value) {
        ...
        nextTick(() => {
          ...
          this.setSelectWordJobData(wordJobDetails.data);
    ...
    setup(props, { emit }) {
      ...
      //사용자 사용 시스템 정보
      const { useMetaMngInstId, useDctnryId, useInfoSysId } =
        userStngInfo.value;
      ...
      const { setSelectWordData, setSelectWordJobData } = actualizingStore;
      ...
      const wordJobGridId = ref('MFGRD230');
      ...
      const getGridInfo = async () => {
        try {
          // 코드명 그리드
          const userGridData = await getUserGridSetting('MFGRD230');
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[wordJobGridId.value]) {
          try {
            ...
            uiStore.setGridColumnDefs('MFGRD230', columnDefs.value);
      ...
      watchEffect(() => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD230) {
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD230;
      ...
      const onWorkJobWordApproval = async () => {
        ...
        console.log('getWordCompliteJobV2-response : ', response);
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD230 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);
        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD230', newColumnFcDefs);
      }
      ...
      const addGridRowData = async (wordResearchQuery) => {
        try {
          ...
          // wordResearchQuery.wordAbbreviationName =
          //   lastItem.wordAbbreviationName;
          // wordResearchQuery.wordDictionaryId = lastItem.wordDictionaryId;
      ...
      const onFilterWindowClosed = async (filterSet) => {
        ...
        if (filterSet) {
          ...
          const gridStorage = JSON.parse(getGridInfoFromStorage());
          console.log('gridStorage.MFGRD230 : ', gridStorage.MFGRD230);
          columnDefs.value = gridStorage.MFGRD230;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD230);
        }
      };
      ...
      // 표준화 단어추출  팝업상태
      const extractWordWindowView = ref(false);
      const onOpenExtractWordWindow = () => {
        popInfo.value.popTitle = '표준화 단어 추출 확인메시지';
        popInfo.value.popMessages =
          '표준화 대상 용어에서 표준화 단어를 추출하는 단계입니다. 표준화 단어를 다시 추출하시겠습니까?';
        extractWordWindowView.value = true;
      };
      const onCloseExtractWordWindow = () => {
        console.log('onCloseExtractWordWindow');
        extractWordWindowView.value = false;
      };

      const onExtractWord = async () => {
        ...
        const response = await getExtractWord(data);
      ...
      // 반응형 선택 단어 데이터
      const selectWordGridData = reactive({});
      ...
      const onApprovalPopup = () => {
        ...
        console.log('filterWordData : ', filterWordData);

        popInfo.value.state = 'redirect';
        popInfo.value.popTitle = '결재 요청 확인';
        popInfo.value.popMessages =
          '<span style="font-weight : 900;">결재 요청이 완료되었습니다.</span> 전자결재 페이지로 이동하여 결재항목을 확인하실 수 있습니다.';
        popInfo.value.confirmBtnText = '전자결재 이동';
        popInfo.value.cancelBtnText = '확인';

        selectWordGridData.value = filterWordData;
      ...
      return {
        ...
        handleScrollChanged,
        setSelectWordJobData,
        extractWordWindowView,
        onOpenExtractWordWindow,
        onCloseExtractWordWindow,
        onExtractWord,
        popInfo,
        useDctnryId,
        getTooltipById,
