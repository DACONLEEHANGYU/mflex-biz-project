<template>
      ...
      <div class="title-s">
        ...
        <!-- <div class="btns" style="display: flex">
          ...
          <button
            ...
            @click="onSelectReg(1)"
            :disabled="
              isSelectType == 'new' || wordDetailsData.jobDivisionCode != 'R'
            "
            title="변경등록"
          >
            변경등록
          ...
          <button
            ...
            :disabled="wordDetailsData.jobDivisionCode != 'R'"
            @click="onSelectReg(4)"
            title="복구등록"
          >
            복구등록
  ...
  <AppWindow
    ...
    height="auto"
  >
    <WordActualizingNameSearchWindow
...
<script setup>
  ...
  import WordActualizingNameSearchWindow from '@/components/popWindow/WordActualizingNameSearchWindow.vue';
  ...
  const { selectWordData, selectWordJobData } = storeToRefs(actualizingStore);
  ...
  const validationMsg = (response) => {
    let errorMessages;

    console.log('validationMsg response : ', response);
  ...
  //   // }
  //   // emit('onWordSave', wordDetailsData);
  ...
  const onUserJobWordUpdate = async () => {
    ...
    switch (wordDetailsData.jobDivisionCode) {
      case 'C':
        jobDivisionCode = 'NEW';
        break;
      case 'U':
  ...
  const onWordNameSearchConfirm = (selectRow) => {
    ...
    wordDetailsData.wordName = selectRow.wordName[0].label;
    wordDetailsData.typeName =
  ...
  const onUserJobReset = () => {
    ...
    if (wordDetailsData.jobDivisionCode === 'C') {
      onSelectReg(0);
    } else {
      onSelectReg(1);
    }
  ...
  const onWordValidationErrorSave = async (data) => {
    ...
    } else if (data.stateName === 'duplicate') {
      wordValidationErrorWindowView.value = false;
      return;
  ...
  //   }
  // });
  watch(selectWordJobData, (newVal) => {
    console.log('wordJobData watch:', newVal);
