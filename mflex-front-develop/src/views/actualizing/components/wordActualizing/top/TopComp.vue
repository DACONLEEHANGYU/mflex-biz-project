...
<script>
  ...
  // import { getWordListV2 } from '@/utils/mflexApi/dictionarySearchApi';
  ...
  import {
    ...
    getMngWordDownload, // 관리 단어 엑셀 다운로드
  ...
  export default {
    ...
    methods: {
      ...
      async onRowClicked(value) {
        ...
        nextTick(() => {
          ...
          if (value.wordSourceCode === 'JOB') {
            this.setSelectWordJobData(wordDetailsDataInfo.data);
          } else {
            this.setSelectWordData(wordDetailsDataInfo.data);
          }
        });
      },
    ...
    setup(props, { emit }) {
      ...
      const { isActualizingWordJobVisible } = storeToRefs(actualizingStore);
      ...
      onBeforeMount(async () => {
        ...
        const researchQuery = {
          ...
          includeJobWord: isActualizingWordJobVisible.value,
          jobTypeCode: 'CUR',
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        // localStorage에 에서 gridData json 파싱, MFGRD220에 대한 값 변경
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD220 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);
        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD220', newColumnFcDefs);
      }
      ...
      watchEffect(async () => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD220) {
          columnDefs.value = gridColumnDefs.value.MFGRD220;
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD220;
          ...
          if (rowFcData.value.length > 0) {
            nextTick(async () => {
              ...
              if (firstRowData) {
                ...
                selectedWordData.value = firstRowData;
                ...
                if (firstRowData.data.wordSourceCode === 'JOB') {
                  setSelectWordJobData(wordDetailsDataInfo.data);
                } else {
                  setSelectWordData(wordDetailsDataInfo.data);
                }
              }
            });
          }
    ...
    function handleSortChanged(newSortedState) {
      ...
      const onFilterWindowClosed = async (filterSet) => {
        ...
        if (filterSet) {
          ...
          const gridStorage = JSON.parse(getGridInfoFromStorage());
          console.log('gridStorage.MFGRD220 : ', gridStorage.MFGRD220);
          columnDefs.value = gridStorage.MFGRD220;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD220);
        }
      };
      ...
      const downloadExcelFromData = async (data, downloadType) => {
        try {
          ...
          worksheet.columns = [
            ...
            { header: '단어영문약어명', key: '단어영문약어명', width: 25 },
            { header: '작업구분명', key: '작업구분명', width: 20 },
            { header: '단어영문명', key: '단어영문명', width: 30 },
            { header: '단어유형', key: '단어유형', width: 20 },
            { header: '도메인분류명', key: '도메인분류명', width: 25 },
            { header: '이음동의어목록', key: '이음동의어목록', width: 25 },
      ...
      // 작업단어 표시 여부 감시
      watch(isActualizingWordJobVisible, async (value) => {
        const researchQuery = {
          instituteId: useMetaMngInstId,
          dictionaryId: useDctnryId,
      ...
      watch(
        ...
        async (value) => {
          ...
          console.log('selectedWordData.value : ', data);
          const selectedRow = document.querySelectorAll(
            '[class~="ag-row-selected"]'
          );
          // 선택된 모든 요소에서 ag-row-selected 클래스를 제거합니다.
          ...
          if (data) {
            ...
            if (data.wordSourceCode === 'JOB') {
              ...
              // null 값으로 중복 데이터 초기화
              setSelectWordJobData(null);
              setSelectWordJobData(termDetailsDataInfo.data);
            } else {
              const wordDetailsQuery = {
                instituteId: data.instituteId,
                dictionaryId: useDctnryId,
              ...
              setSelectWordData(null);
              setSelectWordData(termDetailsDataInfo.data);
            }
          }
          // 초기값으로 설정
