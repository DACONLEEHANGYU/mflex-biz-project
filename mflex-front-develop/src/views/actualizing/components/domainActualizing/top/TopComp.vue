...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onRowClicked(value) {
        ...
        if (value.domainSourceCode === 'MNG') {
          this.setSelectDomainData(null);
          this.setSelectDomainData(value);
        } else {
    ...
    setup(props, { emit }) {
      ...
      const { setSelectDomainData, setSelectDomainJobData } = actualizingStore;

      const { isActualizingDomainJobVisible } = storeToRefs(actualizingStore);
      ...
      const domainSearchGridId = ref('MFGRD240');
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[domainSearchGridId.value]) {
          try {
            ...
            uiStore.setGridColumnDefs('MFGRD240', columnDefs.value);
      ...
      const updateDomainTree = async () => {
        ...
        const domainResearchQuery = {
          ...
          includeJobDomain: isActualizingDomainJobVisible.value,
          jobTypeCode: 'CUR',
          sort: getSortQuery(),
        };
            ...
            nextTick(() => {
              ...
              if (firstRowData) {
                ...
                if (firstRowData.data.domainSourceCode === 'MNG') {
                  setSelectDomainData(firstRowData.data);
                } else {
                  setSelectDomainJobData(firstRowData.data);
                }
              }
            });
          }
      ...
      //   if (storedColumnDefs && storedColumnDefs.MFGRD240) {
      //     uiStore.setGridColumnDefs('MFGRD240', storedColumnDefs.MFGRD240);
      //   }
      // };
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        // localStorage에 에서 gridData json 파싱, MFGRD240에 대한 값 변경
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD240 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);

        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD240', newColumnFcDefs);
      }
      ...
      watchEffect(() => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD240) {
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD240;
      ...
      const onSelectNode = async (value) => {
        ...
        const response = await fetchData(domainResearchQuery);
      };
      ...
      const onFilterWindowClosed = (filterSet) => {
        ...
        if (filterSet) {
          ...
          columnDefs.value = gridStorage.MFGRD240;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD240);
        }
      };
      ...
      // 작업 포함 여부 변경 감지
      watch(isActualizingDomainJobVisible, async (value) => {
        console.log('domainTopComp-isActualizingDomainJobVisible : ', value);
      ...
      const downloadExcelFromData = async (data, downloadType) => {
        try {
          ...
          // 🔥 컬럼 정의 및 헤더 설정
          worksheet.columns = [
            { header: '도메인명', key: '도메인명', width: 30 },
            { header: '작업구분명', key: '작업구분명', width: 25 },
            { header: '도메인분류명', key: '도메인분류명', width: 30 },
            { header: '도메인그룹명', key: '도메인그룹명', width: 30 },
        ...
        async (value) => {
          ...
          if (data) {
            if (data.domainSourceCode === 'MNG') {
              setSelectDomainData(null);
              setSelectDomainData(data);
            } else {
              setSelectDomainJobData(null);
