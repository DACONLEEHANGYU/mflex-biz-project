    ...
    <div class="grid-wrap">
      <div class="grid-top">
        <div class="top-row">
          ...
          />
          <div class="title-btns__row_btween"></div>
        </div>
      </div>
...
<script>
  ...
  import { getTermDetailInfo } from '@/utils/mflexApi/dictionaryManagementApi.js';
  ...
  import {
    ...
    getMngTermDownload, // 관리용어 엑셀다운로드
  ...
  export default {
    ...
    methods: {
      ...
      async onRowClicked(value) {
        ...
        if (value.termSourceCode === 'MNG') {
          ...
          nextTick(() => {
            ...
            this.setSelectTermData(termDetailsData.data);
          });
          ...
          nextTick(() => {
            ...
            this.setSelectTermJobData(termDetailsData.data);
          });
        }
      },
      async onSearchEnter(value) {
        ...
        if (this.searchType === 'natural-query' && value !== '') {
          ...
          //   includeJobTerm: this.isActualizingTermJobVisible,
    ...
    setup(props, { emit }) {
      ...
      const { isTermJobApproval, isTermJobCancel } =
      ...
      const { setSelectTermData, setSelectTermJobData } = actualizingStore;
      const { isActualizingTermJobVisible } = storeToRefs(actualizingStore);
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[tab1GridId.value]) {
          try {
            ...
            uiStore.setGridColumnDefs('MFGRD200', columnDefs.value);
      ...
      //   if (termStoredColumnDefs && termStoredColumnDefs.MFGRD200) {
      //     uiStore.setGridColumnDefs('MFGRD200', termStoredColumnDefs.MFGRD200);
      //   }
      // };
    ...
    */
      watchEffect(async () => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD200) {
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD200;

        console.log(
          'watchEffect - Grid 데이터 설정 전 : ',
          gridColumnDefs.value.MFGRD200
        );
        ...
        try {
          ...
          for (let i = 0; i < terms.length; i++) {
            newGridData.push({
              ...
              relationCodeName: terms[i].relationCodeName,
      ...
      const serarchDataBinding = (termResearchResultData) => {
        ...
        for (let t = 0; t < terms.length; t++) {
          // if (terms[t].term.dictionary.id !== useDctnryId) {
          //   continue; // 조건에 맞지 않으면 건너뜁니다.
          // }
        ...
        if (rowData.value.length > 0) {
          nextTick(async () => {
            ...
            if (firstRowData) {
              ...
              const termBaseQuery = {
                termDictionaryId: firstRowData.data.dctnryId,
                termName: firstRowData.data.termName[0].label,
                termAbbreviationName: firstRowData.data.termEngAbbreviationName,
              };

              const termDetailsDataInfo = await getTermDetailInfo(
                termBaseQuery
              );

              setTermJobData(termDetailsDataInfo.data);
              setSelectTermData(termDetailsDataInfo.data);
            }
          });
        }
      };
      ...
      const onFilterWindowClosed = async (filterSet) => {
        ...
        if (filterSet) {
          ...
          const gridStorage = JSON.parse(getGridInfoFromStorage());
          console.log('gridStorage.MFGRD200 : ', gridStorage.MFGRD200);
          columnDefs.value = gridStorage.MFGRD200;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD200);
        }
      };
        ...
        try {
          if (endScrollStaus === 'Y' && rowData.value != null) {
            ...
            const termResearchQuery = {
              ...
              dictionaryId: useDctnryId,
              includeJobTerm: isActualizingTermJobVisible.value,
              lastItem: {
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        const newColumnFcDefs = newColumnState
          .map((colState) => {
            ...
            return {
              ...
              valueFormatter: valueFormatter,
              cellRenderer:
                colDef.field === 'termName' || colDef.field === 'domainName'
                  ? cellRenderer
                  : null,
        ...
        // localStorage에 에서 gridData json 파싱, MFGRD200에 대한 값 변경
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD200 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);
        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD200', newColumnFcDefs);
      }
      ...
      const onExtractTerm = async () => {
        ...
        if (response.status === 200) {
          ...
          const termExtractData = await getTermListV2(termResearchQuery);

          termDataBinding(termExtractData);
      ...
      const termDataBinding = (termData) => {
        ...
        if (rowData.value.length > 0) {
          nextTick(async () => {
            ...
            selectedTermData.value = firstRowData.data;
            // 첫 행 select 효과
            const nodesWithRowId0 = document.querySelector('[row-id="0"]');
            ...
            if (firstRowData) {
              ...
              if (firstRowData.data.termSourceCode === 'JOB') {
                ...
                nextTick(() => {
                  ...
                  setSelectTermJobData(termDetailsData.data);
                });
                return;
              }
      ...
      watch(isActualizingTermJobVisible, async (value) => {
        console.log('termTopComp-isActualizingTermJobVisible : ', value);
      ...
      return {
        ...
        setTermJobUpdateData,
        popInfo,
        extractTermWindowView,
        onOpenExtractTermWindow,
        onCloseExtractTermWindow,
        onExtractTerm,
