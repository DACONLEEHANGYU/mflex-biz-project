...
<script setup>
  ...
  import DatabaseSchemaComp from '@/views/instSystemMng/components/tab3/DatabaseSchemaComp.vue';
  ...
  const selectInstitute = reactive({
    id: selectDatabaseInstituteId.value, // 초기값으로 현재 사용자의 기관 ID 설정
    name: '',
  });
  ...
  const onResetDetailsInfo = () => {
    ...
    databaseData.instituteId = useMetaMngInstId;
  ...
  watch(selectDatabaseInfo, async (newValue) => {
    ...
    if (!newValue || !newValue.databaseId) {
      console.log('선택된 데이터베이스가 없습니다.');
      onResetDetailsInfo();
      regType.value = 1; // 조회 상태로 변경
      return;
    }
