...
<script setup>
  ...
  onBeforeMount(async () => {
    ...
    //   (items) => items.instituteId === useMetaMngInstId
    // );

    // console.log('selectedInst ============================', selectedInst);

    // if (selectedInst) {
    //   useMetaMngInstId = selectedInst.instituteId;
    ...
    // 사전 조회 API 호출
    let dictinaryListDt = await getDictionaryTree(useMetaMngInstId);
    // 최초 상위사전 명 설정
  ...
  const onSave = async (selectId) => {
    ...
    if (regType.value == '추가') {
      ...
      if (addResult) {
        // 재조회 시간차
        setTimeout(async () => {
          const dictionaryListData = await getDictionaryTree(useMetaMngInstId);
      ...
      if (editResult) {
        setTimeout(async () => {
          ...
          // 수정 후 사전 유형 초기화
          // wordTypeText.value = updatedDictionaryData.type.name;

          // console.log('wordTypeText ========================', wordTypeText);

          // // 수정 후 선택된 사전 id 등록된 사전 id로 변경
          // selectedDictionaryId = updatedDictionaryData.id;
          // dctnryData.value.dictionaryId = updatedDictionaryData.id;
          ...
          // selectedData.value.bgColor =
          //   updatedDictionaryData.type.backgroundColor;
          // selectedData.value.text = updatedDictionaryData.type.name;
          // selectedData.value.type = updatedDictionaryData.type.name;
  ...
  const onDelete = async (deleteId) => {
    ...
    if (removeResult) {
      setTimeout(async () => {
        let dictionaryListData = await getDictionaryTree(useMetaMngInstId);
        const constructTree = constructDictionaryTree(dictionaryListData);
