      ...
      <div class="title-top">
        <div class="title-s">
          ...
          <div class="btns" style="display: flex">
            ...
            <button
              :disabled="isJobCheckd || isErrorState"
        ...
        <div class="title-btns__row_btween">
          <div class="btns">
            <button
              ...
              title="도메인일괄등록"
              @click="onOpenDomainUploadWindow"
            >
              도메인 일괄 등록
            </button>
          </div>
...
<script>
  ...
  import {
    getResearchDomainJob,
    getDomainGroupJobDetails, // 도메인그룹 작업 상세조회
    getDomainClassJobDetails, // 도메인분류 작업 상세조회
    getDomainNameJobDetails, // 도메인명 작업 상세조회
    getCancleDomainGroup, // 도메인그룹 작업 취소
    getCancleDomainClass, // 도메인분류 작업 취소
    getCancleDomainName, // 도메인명 작업 취소
  ...
  import {
    ...
    getJobDomainDownload, // 도메인 작업 엑셀 다운로드
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const domainJobGridId = ref('MFGRD021');
      ...
      const getGridInfo = async () => {
        try {
          // 코드명 그리드
          const userGridData = await getUserGridSetting('MFGRD021');
      ...
      const onSelectionChanged = () => {
        ...
        selectDomainData.value = selectedNodes.map((node) => node.data);
        // 선택된 행에 대한 추가 작업 수행
      };
      ...
      const fetchData = async (resaechQuery) => {
        ...
        console.log('domainJobList : ', domainJobList);
      ...
      const isErrorState = computed(() => {
        if (selectDomainData.value.length === 1) {
          if (selectDomainData.value[0].domainErrorName === '오류') {
            return true;
          }
        }
        return false;
      ...
      const jobDomainSearchResultBinding = (domainJobSearchResult) => {
        console.log(
          'jobDomainSearchResultBinding - domainJobSearchResult : ',
          domainJobSearchResult
        );
      ...
      const onFilterWindowClosed = async (filterSet) => {
        ...
        if (filterSet) {
          ...
          const gridStorage = JSON.parse(getGridInfoFromStorage());
          console.log('gridStorage.MFGRD021 : ', gridStorage.MFGRD021);
          columnDefs.value = gridStorage.MFGRD021;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD021);
        }
      };
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD021 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);
        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD021', newColumnFcDefs);
      }
      ...
      const onApprovalPopup = () => {
        ...
        selectDomainData.value = filterDomainData;
      ...
      const downloadExcelFromData = async (data, downloadType) => {
        try {
          ...
          worksheet.columns = [
            ...
            { header: '도메인그룹명', key: '도메인그룹명', width: 20 },
