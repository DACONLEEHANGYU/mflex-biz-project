        ...
        <div class="title-s">
          ...
          <div class="btns" style="display: flex">
            ...
            <button
              ...
              title="작업완료"
              :disabled="termJobSelected.length < 1 || isErrorState"
        ...
        <div class="title-btns__row_btween">
          <div class="btns">
            <button
              ...
              title="일괄영문약어생성"
              :disabled="
                !termJobSelected.some(
                  (job) => job.data.applicationCategory === '신규'
                )
              "
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async onSearchEnter(value) {
        ...
        if (this.searchType === 'natural-query' && value !== '') {
          ...
          const termJobSearchResult = await getTermJobListV2(termJobQuery);
          console.log('searchResult : ', termJobSearchResult);
    ...
    setup(props, { emit }) {
      ...
      const { isTermJobSave } = storeToRefs(dictionaryMngStore);
      const {
      ...
      const termJobGridId = ref('MFGRD031');
      ...
      const getGridInfo = async () => {
        try {
          // 코드명 그리드
          const userGridData = await getUserGridSetting('MFGRD031');
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[termJobGridId.value]) {
          try {
            ...
            uiStore.setGridColumnDefs('MFGRD031', columnDefs.value);
      ...
      const isErrorState = computed(() => {
        if (selectTermData.value.length === 1) {
          if (selectTermData.value[0].termNameErrorName === '오류') {
            return true;
          }
        }
        return false;
      ...
      const onSelectionChanged = () => {
        ...
        selectTermData.value = selectedNodes.map((node) => node.data);
      };
      ...
      watchEffect(() => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD031) {
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD031;
      ...
      const fetchData = async () => {
        ...
        for (let i = 0; termJobs.length > i; i++) {
          ...
          termJobData.push({
            ...
            termNameErrorCode: termJobs[i].termNameErrorCode,
      ...
      const onCancel = async (state) => {
        ...
        if (state === 0) {
          ...
          await getCompleteTermJob(termCancelParamData);
        }
      ...
      const onAllEngCreateSave = async (params) => {
        ...
        console.log('onAllEngCreateSave - params : ', params);
      ...
      const onFilterWindowClosed = async (filterSet) => {
        ...
        if (filterSet) {
          ...
          const gridStorage = JSON.parse(getGridInfoFromStorage());
          console.log('gridStorage.MFGRD017 : ', gridStorage.MFGRD031);
          columnDefs.value = gridStorage.MFGRD031;

          gridApi.value.setGridOption('columnDefs', gridStorage.MFGRD031);
        }
      };
      ...
      function handleColumnStateChanged(newColumnState) {
        ...
        // localStorage에 에서 gridData json 파싱, MFGRD031에 대한 값 변경
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        gridStorage.MFGRD031 = newColumnFcDefs;
        // 로컬스토레지에 저장
        saveGridInfoToStorage(gridStorage);
        // 스토어에 저장
        uiStore.setGridColumnDefs('MFGRD031', newColumnFcDefs);
      }
      ...
      const addGridRowData = async (termResearchQuery) => {
        try {
          ...
          for (let i = 0; i < terms.length; i++) {
            if (terms[i].jobDivisionName === '신규등록') {
              applicationCategory = '신규';
            } else if (terms[i].jobDivisionName === '변경등록') {
              applicationCategory = '변경';
            } else if (terms[i].jobDivisionName === '폐기등록') {
              applicationCategory = '폐기';
            } else if (terms[i].jobDivisionName === '복구등록') {
              applicationCategory = '복구';
            } else if (terms[i].jobDivisionName === '삭제등록') {
              applicationCategory = '삭제';
            } else {
              applicationCategory = terms[i].jobDivisionName;
            }
            ...
            newGridData.push({
              ...
              termBasicErrorCode: terms[i].termBasicErrorCode,
              termNameErrorCode: terms[i].termNameErrorCode,
              termNameErrorName: terms[i].termNameErrorName,
              termNameErrorYn: terms[i].termNameErrorYn,
              wordAbbreviationCombinationName:
                terms[i].wordAbbreviationCombinationName,
              wordAbbreviationErrorCode: terms[i].wordAbbreviationErrorCode,
              wordCombinationName: terms[i].wordCombinationName,
              wordErrorCode: terms[i].wordErrorCode,
      ...
      const onApprovalPopup = () => {
        ...
        selectTermData.value = filterTermData;
