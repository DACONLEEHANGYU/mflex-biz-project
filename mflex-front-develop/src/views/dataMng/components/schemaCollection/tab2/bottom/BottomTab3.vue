...
<script>
  ...
  import { getProcedureCollectionList } from '@/utils/mflexApi/dataMng/schemaCollectionApi';
  ...
  export default {
    ...
    methods: {
      ...
      async onSearchEnter(value) {
        ...
        if (this.searchType === 'natural-query' && value !== '') {
          ...
          const tableResearchResult = await getProcedureCollectionList(
            researchQuery
          );
          ...
          const collectionResearchResult = await getProcedureCollectionList(
            researchQuery
          );
    ...
    setup(props) {
      ...
      const { selectCollection, isExcludeProcedure } = storeToRefs(
        useSchemaCollectionStore()
      );
      const { setSelectProcedureList, setIsExcludeProcedure } =
        useSchemaCollectionStore();
      ...
      const tab3GridId = 'MFGRD073';
      const gridInfoDefs = ref({
        scrnGridId: tab3GridId,
        scrnId: '',
      });
      ...
      const transformGridData = (data) => {
        ...
        const fieldMapping = {
          NO: 'no',
          SCHM_NM: 'schemaName',
          PRCDR_NM: 'procedureName',
          PRCDR_EXPLN: 'procedureExplain',
      ...
      const getGridInfo = async () => {
        try {
          const userGridData = await getUserGridSetting(tab3GridId);
          ...
          return transformedDataWithNo; // transMFGRD073formedData를 반환
        } catch (error) {
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[tab3GridId]) {
          try {
            ...
            uiStore.setGridColumnDefs('MFGRD073', columnDefs.value);

            gridStorage[tab3GridId] = transformedData;
            // gridStorage[tab3GridId] = transformedData;
          ...
          columnDefs.value = gridStorage[tab3GridId];
        }
      ...
      const fetchData = async (data) => {
        ...
        const response = await getProcedureCollectionList(data);
        console.log('response-getProcedureCollectionList ', response);
        ...
        for (let i = 0; i < response.items.length; i++) {
          ...
          tempData.push({
            ...
            procedureExplain: item.procedureExplain,
      ...
      const addGridRowData = async (lastItem) => {
        try {
          ...
          researchQuery.value.lastItem = lastItem;
          researchQuery.value.sort = getSortQuery();
          const response = await getProcedureCollectionList(
            researchQuery.value
          );
          ...
          for (let i = 0; i < collectionList.length; i++) {
            newGridData.push({
              ...
              procedureName: collectionList[i].procedureName,
              procedureExplain: collectionList[i].procedureExplain,
      ...
      const handleSortChanged = async (newSortedState) => {
        ...
        if (sortState.value.length == 0) {
          ...
          const sortData = await getProcedureCollectionList(sortResarchQuery);
          ...
          const sortData = await getProcedureCollectionList(sortResearchQuery);
          console.log('sortData : ', sortData);
      ...
      const searchDataBinding = (procedureList) => {
        console.log('procedureList : ', procedureList);

        const collectionList = procedureList.items;

        if (collectionList.length < 0) {
          resultCount.value.count = 0;
          resultCount.value.total = 0;
        } else {
          resultCount.value.count = procedureList.searchCount;
          resultCount.value.total = procedureList.totalCount;
      ...
      watch(
        ...
        async (value) => {
          console.log('Bottom3-selectCollection.value ===', value);
          ...
          researchQuery.value.collectionId = value.collectionId;
      ...
      watchEffect(() => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD073) {
          return;
        }
        columnDefs.value = gridColumnDefs.value.MFGRD073;
      ...
      const onSetUserGridSetting = async () => {
        ...
        const gridStorage = JSON.parse(getGridInfoFromStorage());
        console.log('gridStorage[tab1GridId] : ', gridStorage[tab3GridId]);
        const fieldMapping = {
          no: 'NO',
          schemaName: 'SCHM_NM',
          procedureName: 'PRCDR_NM',
          procedureExplain: 'PRCDR_EXPLN',
          collectionExcludeYnName: 'CLCT_EXCL_YN',
          collectionExclusionReasonCode: 'CLCT_EXCL_RSN_CN',
        ...
        await setUserGridSetting(tab3GridId, newGridStting);
      ...
      const onSearchRemove = async () => {
        ...
        // 그리드 기본값 호출
        const gridDefaultData = await getGridDefaultData(tab3GridId);

        console.log('gridDefaultData : ', gridDefaultData);

        // 기본값 설정
        await setUserGridSetting(tab3GridId, gridDefaultData);
        ...
        uiStore.setGridColumnDefs(tab3GridId, columnDefs.value);
        gridApi.value.setGridOption('columnDefs', columnDefs.value);
      ...
      const onSelectionChanged = (selectRow) => {
        ...
        setSelectProcedureList(selectedRows);
      };

      // 수집제외 설정 시 재조회
      watch(isExcludeProcedure, async (value) => {
        console.log('isExcludeTable.value ===', value);
        if (value) {
          await fetchData(researchQuery.value);
          setIsExcludeProcedure(false);
        }
      });
