...
<script>
  ...
  // 분리된 컴포저블들
  import { useSchemaSearchTab4State } from '@/composables/dataMng/schemaSearch/tab4/useSchemaSearchTab4State.js';
  import { useSchemaSearchTab4Data } from '@/composables/dataMng/schemaSearch/tab4/useSchemaSearchTab4Data.js';
  import { useSchemaSearchTab4Handlers } from '@/composables/dataMng/schemaSearch/tab4/useSchemaSearchTab4Handlers.js';
  import { useSchemaSearchTab4Utils } from '@/composables/dataMng/schemaSearch/tab4/useSchemaSearchTab4Utils.js';
  ...
  export default {
    ...
    beforeMount() {
      ...
      console.log('foreignKey beforeMount  ========================');
    },
    ...
    setup(props, { emit }) {
      ...
      const { setForeignKeyData } = useSchemaSearchTabStore();
      ...
      // COMPOSABLES
      // ============================
      const state = useSchemaSearchTab4State();
      const dataHandlers = useSchemaSearchTab4Data();
      const eventHandlers = useSchemaSearchTab4Handlers();
      const utils = useSchemaSearchTab4Utils();
      ...
      const handlers = {
        ...
        getGridInfo: utils.getGridInfo(state.tab4GridId),
      ...
      const initializeGridColumnDefs = () => {
        ...
        if (storedColumnDefs && storedColumnDefs[state.tab4GridId.value]) {
          uiStore.setGridColumnDefs(
            state.tab4GridId.value,
            storedColumnDefs[state.tab4GridId.value]
          );
        }
      };
      ...
      state.columnDefs.value =
        gridColumnDefs.value[state.tab4GridId.value] || [];
      ...
      onBeforeMount(async () => {
        ...
        if (!gridStorage[state.tab4GridId.value]) {
          ...
          state.columnDefs.value = gridStorage[state.tab4GridId.value];
        }
      });
      ...
      watchEffect(() => {
        ...
        if (!gridColumnDefs.value || !gridColumnDefs.value.MFGRD063) {
          return;
        }
        state.columnDefs.value = gridColumnDefs.value.MFGRD063;
